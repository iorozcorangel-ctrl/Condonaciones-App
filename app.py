...
def init():
    defs = {
        ...
        "dias_especiales":   set(),
        "paso":              "inicio",   # inicio | confirmacion | reporte
        "desfases":          {},
        "montos":            {},
...
if not st.session_state["autenticado"]:
    ...
    with c2:
        st.markdown("<h2 style='text-align:center;color:#1F4E79;'>🚢 Sistema de Condonaciones</h2>", unsafe_allow_html=True)
        st.markdown("<p style='text-align:center;color:#666;'>Terminal Portuaria Pacífico</p>", unsafe_allow_html=True)
        with st.form("login"):
...
with col_izq:
    ...
    # ── Perfil ────────────────────────────────────────────────
    st.markdown("<div class='sec-hdr'>Perfil de Condonación</div>", unsafe_allow_html=True)
    ...
    with c1:
        sel = st.selectbox("Perfil", nombres, index=idx,
                           ...
                           disabled=st.session_state["paso"] != "inicio")
    ...
    with c3:
        if st.button("✏️ Editar", use_container_width=True,
                     disabled=nuevo_idx == 0 or st.session_state["paso"] != "inicio"):
    ...
    if st.session_state["mostrar_form_perfil"]:
        ...
        with st.expander("Configurar Perfil", expanded=True):
            ...
            r1 = st.checkbox("Aplicar regla 30 días (Regla 1)", value=p.get("regla1_activa", True))
            r2 = st.checkbox("Aplicar regla 4 días (Regla 2)",  value=p.get("regla2_activa", True))
            ...
            if bg1.button("💾 Guardar", use_container_width=True):
                if nombre_p:
                    np = {"nombre": nombre_p, "es_default": False,
                          "regla1_activa": r1, "regla2_activa": r2,
                          "dias_previo": dp, "dias_ferromex": df, "dias_carretero": dc}
                    if modo == "nuevo":
                        st.session_state["perfiles"] = agregar_perfil(np, st.session_state["perfiles"])
                        st.session_state["perfil_idx"] = len(st.session_state["perfiles"]) - 1
                    else:
                        st.session_state["perfiles"] = modificar_perfil(nuevo_idx, np, st.session_state["perfiles"])
    ...
    # ── Datos de solicitud ────────────────────────────────────
    st.markdown("<div class='sec-hdr'>Datos de Solicitud</div>", unsafe_allow_html=True)
    dc1, dc2 = st.columns(2)
    nc_input    = dc1.text_input("N° Nota de Crédito", placeholder="Ej: NC-2585",
                                  disabled=st.session_state["paso"] != "inicio")
    ...
    with fa1:
        f_tab = st.file_uploader("📋 Tabulador Comercial", type=["xlsx","xls"],
                                  disabled=st.session_state["paso"] != "inicio")
        if f_tab:
            try:
                ...
                st.error(str(e))
    with fa2:
        f_bi = st.file_uploader("📊 Archivo BI", type=["xlsx","xls"],
                                 disabled=st.session_state["paso"] != "inicio")
        if f_bi:
            try:
    ...
    # ── Botones principales ───────────────────────────────────
    st.markdown("---")
    ambos = st.session_state["df_tab"] is not None and st.session_state["df_bi"] is not None

    bb1, bb2 = st.columns(2)
    with bb1:
        iniciar = st.button("▶ Iniciar Análisis",
                            disabled=not ambos or st.session_state["paso"] != "inicio",
                            use_container_width=True, type="primary")
    with bb2:
        if st.button("↺ Nuevo Análisis", use_container_width=True):
            for k in ["df_tab","df_bi","df_tab_v","df_bi_v","desfases","montos"]:
    ...
    if iniciar:
        ...
        with st.spinner("Calculando desfases..."):
            desfases = calcular_desfases(df_bv, st.session_state["dias_especiales"], perfil)
        ...
        if no_r1:
            st.warning(f"**Regla 1 — 30 días:** Contenedores fuera de plazo: {', '.join(no_r1)}")

        # Regla 2
        no_r2 = aplicar_regla2(df_bv, perfil)
        if no_r2:
            st.warning(f"**Regla 2 — 4 días:** Contenedores que no cumplen: {', '.join(no_r2)}")

        # Alertas liner y reprogramaciones
        liners, reprog = [], []
        for _, row in df_bv.iterrows():
            cont = row[COL_BI["contenedor"]]
            fa = row.get(COL_BI["fecha_auth_naviera"])
            if pd.notna(fa) and str(fa).strip() not in ("","nan"):
                liners.append(cont)
            try:
                ...
                pass
        if liners:
            st.info(f"**Liner:** Revisar autorización naviera para: {', '.join(liners)}")
        if reprog:
            st.warning(f"**Reprogramaciones:** Más de 1 programación en: {', '.join(reprog)}")

        # Guardar en sesión y pasar a confirmación
    ...
    if st.session_state["paso"] == "ajuste":
        ...
        for cont in contenedores:
            ...
            with ac3:
                if not checks[cont]:
                    entries[cont] = st.number_input("Días", min_value=0,
                                                     value=0, key=f"d_{cont}",
                                                     label_visibility="collapsed")
    ...
    if st.session_state["paso"] == "reporte":
        ...
        # Alertas finales
        sin_alm = [row[COL_BI["contenedor"]] for _, row in df_bv.iterrows()
                   if float(row.get(COL_BI["alm_qty"], 0) or 0) <= 0]
        if sin_alm:
            st.warning(f"Sin almacenajes cobrados para: {', '.join(sin_alm)}")

        admon_sin = [row[COL_BI["contenedor"]] for _, row in df_bv.iterrows()
                     if float(row.get(COL_BI["alm_qty"], 0) or 0) <= 0
                     and float(row.get(COL_BI["admon_qty"], 0) or 0) > 0]
        if admon_sin:
            st.warning(f"Cobro de Admin y Control sin almacenajes para: {', '.join(admon_sin)}")
...
with col_der:
    ...
    for semana in semanas:
        ...
        for i, dia in enumerate(semana):
            ...
            label = f"{bg}{dia}"
            if dcols[i].button(label, key=f"c_{fecha}",
                                use_container_width=True):
    ...
    if st.session_state["dias_especiales"]:
        fechas_str = ", ".join(sorted(
            d.strftime("%d/%m/%Y") for d in st.session_state["dias_especiales"]
        ))
